"""
Locust load tests for the Product Search API.

Uses FastHttpUser with minimal wait time to maximize load per user.

Test 1 - Baseline:       5 users, 2 minutes
  locust -f search_locustfile.py --host http://100.24.62.239:8080 \
    --headless -u 5 -r 5 --run-time 2m

Test 2 - Breaking Point: 20 users, 3 minutes
  locust -f search_locustfile.py --host http://100.24.62.239:8080 \
    --headless -u 20 -r 20 --run-time 3m
